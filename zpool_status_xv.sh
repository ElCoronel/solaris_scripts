...
### if error file does not exist, create, log error and send email
                                else
...
### if error condition does not exist log healthy status
                else
                        if [ ! -d $LOGDIR/$REMHOST ]; then
                                mkdir $LOGDIR/$REMHOST
                        fi
