...
for i in `cat $HOSTLIST | awk '{print $1}'`
        do
                ...
                if [[ -z $(ssh rmtmon@$i /usr/sbin/zpool status -xv | grep "all pools are healthy") ]]; then
### if error exists, loop through errored pool
                        ZPOOL=(`ssh rmtmon@$i /usr/sbin/zpool status -xv | /usr/gnu/bin/grep 'pool:' | awk '{print $2}'`)
                        for a in "${ZPOOL[@]}"
                                do
### if error file exists log dated error
                                if [[ -f $LOGDIR/$REMHOST/$a-error.out ]]; then
                                        echo "$i $REMHOST - `date` - `ssh rmtmon@$i /usr/sbin/zpool status -xv | grep $a | awk '{print $2}' | grep $a`" >> $LOGDIR/$REMHOST/zpoolxv.$(date +%Y-%m).log
### if error file does not exist, create, log date error and send email
                                        ...
                                        ssh rmtmon@$i /usr/sbin/zpool status -xv | /usr/gnu/bin/grep -A 3 "pool: $a" >> $LOGDIR/$REMHOST/$a-error.out
                                         echo "$i $REMHOST - `date` - `ssh rmtmon@$i /usr/sbin/zpool status -xv | grep $a | awk '{print $2}' | grep $a`" >> $LOGDIR/$REMHOST/zpoolxv.$(date +%Y-%m).log
                                        cat $LOGDIR/$REMHOST/$a-error.out | mailx -s "zpool status error $REMHOST $(date '+%F %H:%M')" greg.sanders@opm.gov
                                fi
                        done
### if error condition does not exist log dated healthy status
                        ...
                        echo "$i $REMHOST - `date` - `ssh rmtmon@$i /usr/sbin/zpool status -xv`" >> $LOGDIR/$REMHOST/zpoolxv.$(date +%Y-%m).log
                fi
        done
### clean up old logs
### need to edit
